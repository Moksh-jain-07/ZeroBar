...
public class ApiController {
    ...
    public ResponseEntity<?> demoSend(@RequestBody DemoSendRequest req) throws Exception {
        ...
        String startDevice = req.startDevice == null ? "phone-mumbai" : req.startDevice;
    ...
    /**
     * Aggregate metrics across all ever-processed packets.
     * Duplicate drop rate = duplicates seen / total claimed hashes.
     */
    @GetMapping("/stats")
    public Map<String, Object> stats() {
        long totalCached   = idempotency.size();
        long totalSettled  = txRepo.countSettled();
        long totalDupes    = Math.max(0, totalCached - totalSettled);
        BigDecimal totalAmount = txRepo.sumSettledAmount();
        Double avgHops     = txRepo.avgHopCount();

        double dupeRate = totalCached == 0 ? 0.0
                : BigDecimal.valueOf(totalDupes * 100.0 / totalCached)
                            .setScale(1, RoundingMode.HALF_UP).doubleValue();

        return Map.of(
                "totalPacketsProcessed", totalCached,
                "totalSettled",          totalSettled,
                "totalDuplicatesDropped",totalDupes,
                "duplicateDropRatePct",  dupeRate,
                "totalAmountSettled",    totalAmount,
                "avgHopsPerSettlement",  avgHops == null ? 0 : BigDecimal.valueOf(avgHops).setScale(1, RoundingMode.HALF_UP)
        );
    }
}
